...
def doGoogleSearch(): #define the fn here
    ...
    for link in soup.find_all("a"): #foreach link in the page html
        if("/url?q=" in str(link)):
            #if the html line contains url then keep and process it
            link_href=str(link.get("href")) #get link to the page
            cutoff=link_href.find("&sa=U&ved")
                #get rid of useless end piece
            Site_Link=link_href[7:cutoff]
                #get rid of useless bit at start
            text.insert(INSERT,Site_Link)
...
Form=Tk()
    #registers form as the fn which will be called from the TK mainloop
Form.geometry("800x500")
    #make the form [x,y] in size

#canvas
canvas=Canvas(Form, width=400,height=300)
    # canvas with dimensions [400,300]
canvas.pack()
    # add the canvas to the form, Form
...
#text to tell the user how many links there are
text2=Text(Form)
canvas.create_window(50,100,window=text2)
